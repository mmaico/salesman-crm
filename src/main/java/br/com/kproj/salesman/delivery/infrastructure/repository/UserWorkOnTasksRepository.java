...
public interface UserWorkOnTasksRepository extends BaseRepositoryLegacy<UserEntity, Long> {
    ...
    Long countOnAllTasksBy(@Param("user") UserEntity user, @Param("status") TaskStatusEntity status);
    ...
    Long countOnTaskBy(@Param("user")UserEntity user, @Param("salesOrder") SalesOrderEntity salesOrderEntity, @Param("status") TaskStatusEntity status);
}
